...
describe("scenariosFromPlaywrightReport", () => {
  ...
  it("reports a serial re-run that passed every time as passed, with the last attempt's duration", () => {
    // A failing sibling in a `serial` spec re-runs the whole chain; the earlier scenarios
    // pass again without ever having failed. Playwright calls that "expected"; so do we.
    const report = {
      suites: [
        {
          specs: [
            {
              title: "scenario 1: staff opens a call for proposals",
              tests: [
                {
                  status: "expected",
                  results: [
                    { status: "passed", duration: 173, retry: 0 },
                    { status: "passed", duration: 193, retry: 1 },
                  ],
                },
              ],
            },
    ...
    expect(scenariosFromPlaywrightReport(report)).toEqual([
      {
        name: "scenario 1: staff opens a call for proposals",
        status: "passed",
        durationSeconds: 0.193,
      },
    ]);
  });
...
describe("buildRunSummary", () => {
  ...
  it("stays inside the ~1 KB budget for a full regression run", () => {
    const s = buildRunSummary({
      ...
      runUrl: "https://github.com/noirlab/gpp-tests/actions/runs/1234",
    });
