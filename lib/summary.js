 ...
 *
 * A test that passed on *every* attempt is `passed`, even with several attempts: in a
 * `serial` spec a failing sibling makes Playwright re-run the whole chain, so the earlier
 * scenarios pass twice without ever having failed. Playwright itself calls that `expected`;
 * the 2026-09-15 ledger called it flaky and made a one-test failure look like four. For such
 * a re-run the duration is the final attempt's — the re-run says nothing about the scenario's
 * own cost — while a genuinely flaky test keeps the sum of its attempts, the time it took to
 * get to green.
 *
  ...
  const walk = (suite) => {
    for (const spec of suite?.specs ?? []) {
      for (const test of spec.tests ?? []) {
        ...
        const status = playwrightStatus(test, results);
        const counted =
          status === "passed" && results.length > 1 ? results.slice(-1) : results;
        ...
        out.push({ name: spec.title, status, durationSeconds });
      }
    }
...
function playwrightStatus(test, results) {
  ...
  const passedSome = attempts.some((r) => r.status === "passed");
  const failedSome = attempts.some((r) => r.status !== "passed");
  if (!passedSome) return "failed";
  return failedSome ? "flaky" : "passed";
}
